...
CONNECTION_STRING = f"mongodb+srv://padd:{MONGO_PASSWORD}@cluster0.vb9sw.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"
...
✏️ Mandatory Tasks:
- Follow @PaddFinance on twitter
- Like and retweet our pinned post and tag 3 friends with the hash tag #PaddFinance
- Join our Telegram Group https://t.me/paddFi
- Join our Telegram Announcement Channel https://t.me/paddfinance
- Complete the bot

NOTE: Users found Cheating would be disqualified & banned immediately.

Airdrop Date: *After round two airdop program*{EXPLORER_URL}
{WEBSITE_URL}
"""

MAKE_SURE_TELEGRAM = f"""
🔸 Follow our telegram channels
Make sure you join our Telegram channels
{TELEGRAM_LINKS}
...
SUBMIT_BEP20_TEXT = f"""
Type in *your Eth Wallet Address*

Please make sure your wallet supports the *{AIRDROP_NETWORK}*
"""


SUBMIT_TELEGRAM_QUOTE_TEXT = f"""
Enter in *your Twitter Quote Link*

Please make sure the link is valid else you would not receive the airdrop
"""
...
WITHDRAWAL_TEXT = f"""
Withdrawals would be sent out automatically to your {AIRDROP_NETWORK} address after the round two airdrop program
NOTE: Users found Cheating would be disqualified & banned immediately."""
...
def follow_telegram(update, context):
    ...
    update.message.reply_text(text="After completing the above task, Please click on \"Done\" to proceed", parse_mode=telegram.ParseMode.MARKDOWN, reply_markup=ReplyKeyboardMarkup(
        [["Done"], ["Cancel"]]
    ))
...
def submit_quote(update, context):
    update.message.reply_text(text=SUBMIT_TELEGRAM_QUOTE_TEXT, parse_mode=telegram.ParseMode.MARKDOWN)
    update.message.reply_text(text="Type in *the link* to proceed", parse_mode=telegram.ParseMode.MARKDOWN, reply_markup=ReplyKeyboardMarkup(
        [["Cancel"]]
    ))
...
def submit_address(update, context):
    ...
    user = update.message.from_user
    if not user.id in USERINFO:
        print("user id not in user info!!!!!!")
    ...
    print("Returning end conversation!!!!!!111")
...
PROCEED, FOLLOW_TELEGRAM, SUBMIT_QUOTE, FOLLOW_TWITTER, SUBMIT_ADDRESS, END_CONVERSATION, LOOP, SUREWANTTO, CAPTCHASTATE = range(9)
...
states = {
    ...
    SUBMIT_QUOTE: [cancelHandler, MessageHandler(Filters.text, submit_quote)],
